...
socket.on('userLeft', (userId) => {
  ...
  userItems.forEach((item) => {
    if (item.textContent.includes(userId)) {
      item.remove(); // Remove the user from the user list
    }
  });
});

socket.on('roomDeleted', (roomId) => {
  // Remove the deleted room from the list of available rooms displayed on the client
  const roomItems = document.querySelectorAll('#showRoom li');
  roomItems.forEach((item) => {
    if (item.textContent.includes(roomId)) {
      item.remove(); // Remove the room from the list
    }
  });
});
